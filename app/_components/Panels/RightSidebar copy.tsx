...
import { useHandleChangeColor } from "../../_hooks/useHandleChangeColor";
import { useHandlePressEnterKey } from "../../_hooks/useHandlePressEnterKey";
...
const extractMaterialSnapshot = (material: THREE.Material) => {
  const base = {
...
const buildMaterialParams = (
  ...
  const allow = {
    MeshBasicMaterial: ["color", "wireframe", "transparent", "opacity", "side"],
    MeshLambertMaterial: [
      "color",
      "wireframe",
      "transparent",
...
export default function RightSidebar() {
  ...
  const handleChangeMaterial = (type: MaterialType) => {
    ...
    const snapshot = extractMaterialSnapshot(oldMaterial);
    console.log("snapshot", snapshot);
    const params = buildMaterialParams(type, snapshot);
    ...
    setGeometryMaterial({
      ...
      props: { wireframe: false, opacity: 1, transparent: false },
    });
    setGeometryVisibility(true);
  };

  const applyMaterialProperty = (
    ...
    const materialAsRecord = mat as unknown as Record<string, unknown>;
  ...
  const handleFogIntensityChange = (value: number) => {
    setSceneFog({ density: value });
  };
  const handleFogNearChange = (value: number) => {
    setSceneFog({ near: value });
  };
  const handleFogFarChange = (value: number) => {
    setSceneFog({ far: value });
  };
  ...
  // na2s el lights ll geometry el selected we raycasting ll geometry = false when not visible we el lock fe left sidebar
  // reset properties of materials based on geometry id

  // later
  // ortho camera controls zoom level display and itself
            ...
            <>
              ...
              {sceneFog.type === "FogExp2" ? (
                <InputSlider
                  ...
                  onFogChange={handleFogIntensityChange}
                />
              ) : (
                <>
                  <InputSlider
                    ...
                    onFogChange={handleFogNearChange}
                  />
                  ...
                  <InputSlider
                    ...
                    onFogChange={handleFogFarChange}
                  />
                </>
              )}
